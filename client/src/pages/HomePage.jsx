...
const HomePage = () => {
  ...
  const formatProjectTime = (date) => {
    const safeDate = date ? moment(date) : null
    return safeDate && safeDate.isValid() ? safeDate.format('MMM D, YYYY h:mm A') : 'Recently'
  }
    ...
    <div className="h-screen overflow-y-scroll text-white font-sans bg-[url('/bg-img.png')] bg-cover bg-center bg-no-repeat">
      <nav className="sticky top-0 z-10 flex items-center justify-between px-6 py-4">
        <div className=" flex items-center gap-2">
          ...
          <span className="text-xl font-semibold tracking-tight">Builder</span>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 pb-20 mt-8 xl:mt-28">
        <div className="w-full max-w-2xl flex flex-col items-center">
          <div className="flex items-center gap-2 p-2 pr-3 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-[13px] text-white/90">
          <span>CREATE YOUR FIRST PROJECT FOR FREE</span>
           ...
           <p className='text-center text-sm md:text-base max-w-xl mt-4 text-white/65 leading-relaxed'> 
             ...
             Describe your idea and watch the AI design, structure and launch your website instantly. No coding required.
           </p>
           ...
           <div className='masked-marquee w-full mt-4 max-w-2xl overflow-hidden py-1'>
            ...
            <div className='animate-marquee gap-3'>
              {homeTags.map((tag,i)=>(
                ...
                className='px-4 py-1.5 border rounded-full text-sm text-white bg-white/10 border-white/25 hover:bg-white/20 transition  cursor-pointer shirnk-0 font-medium'>
                  {tag}
                </button>
            ...
            <div className='mt-12 w-full'>
              ...
              <span className='text-xs text-zinc-100 font-normal'>
               ...
               <div className='space-y-2  max-h-[80vh] overflow-y-auto pr-1'>
                {projects.map((p)=> (
                  <div key={p._id} className='bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex items-center justify-between group hover:border-white/20 cursor-pointer backdrop-blur-md transition-all' 
